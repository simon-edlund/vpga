import { defineStore } from 'pinia'
import { ref, watch } from 'vue'

const translations = {
  sv: {
    language: 'Språk',
    swedish: 'Svenska',
    english: 'Engelska',
    standings: 'Ställning',
    rounds: 'Rundor',
    members: 'Medlemmar',
    enterScores: 'Registrera scorer',
    changePassword: 'Byt lösenord',
    logout: 'Logga ut',
    seasonStandings: 'Säsongsställning',
    season: 'Säsong',
    loading: 'Laddar...',
    noRoundsForSeason: 'Inga rundor har skapats för {season} än.',
    player: 'Spelare',
    best4: 'Bästa 4',
    best4Legend: 'De fyra bästa resultaten markeras i grönt.',
    absentLegend: 'Frånvarande spelare får högsta score i rundan + 1.',
    noSeasonsFound: 'Inga säsonger hittades. En administratör behöver skapa rundor först.',
    round: 'Runda',
    date: 'Datum',
    course: 'Bana',
    notes: 'Noteringar',
    viewScores: 'Visa scorer',
    backToRounds: 'Tillbaka till rundor',
    netStrokes: 'Nettoslag',
    absent: 'Frånvarande',
    noScoresYet: 'Inga scorer har registrerats för denna runda än.',
    email: 'E-post',
    password: 'Lösenord',
    continue: 'Fortsätt',
    firstLoginHelp: 'Vid första inloggningen anger du din e-post och fortsätter. Vi skickar en verifieringslänk för att välja lösenord.',
    firstLoginEmailSent: 'Vi har skickat en verifieringslänk till {email}. Länken gäller i 24 timmar.',
    passwordSetupFromEmail: 'Du öppnade verifieringslänken. Välj ditt nya lösenord för att fortsätta.',
    forgotPassword: 'Jag har glömt mitt lösenord',
    enterEmailFirst: 'Ange din e-postadress först',
    passwordResetEmailSent: 'Om adressen finns registrerad har en återställningslänk skickats. Länken gäller i 24 timmar.',
    couldNotSendPasswordResetEmail: 'Kunde inte skicka återställningsmail',
    newPassword: 'Nytt lösenord',
    repeatPassword: 'Upprepa lösenord',
    setPassword: 'Välj lösenord',
    invalidEmailOrPassword: 'Fel e-post eller lösenord',
    passwordsDoNotMatch: 'Lösenorden matchar inte',
    couldNotCompleteFirstLogin: 'Kunde inte slutföra första inloggningen',
    currentPassword: 'Nuvarande lösenord',
    repeatNewPassword: 'Upprepa nytt lösenord',
    passwordChanged: 'Lösenordet har ändrats',
    errorChangingPassword: 'Fel vid ändring av lösenord',
    save: 'Spara',
    name: 'Namn',
    addMember: 'Lägg till medlem',
    fullName: 'Fullständigt namn',
    golfId: 'Golf-id',
    handicap: 'Handicap',
    admin: 'Admin',
    verified: 'Verifierad',
    active: 'Aktiv',
    yes: 'Ja',
    no: 'Nej',
    cancel: 'Avbryt',
    edit: 'Redigera',
    resetLogin: 'Nollställ inloggning',
    delete: 'Radera',
    deleteMemberConfirm: 'Radera medlemmen "{name}"? Detta tar också bort spelarens scorer.',
    resetLoginConfirm: 'Nollställ inloggning för "{name}"? Medlemmen måste slutföra första inloggningen igen.',
    manageRounds: 'Hantera rundor',
    addRound: 'Lägg till runda',
    roundNumber: 'Runda #',
    noRoundsYet: 'Inga rundor än.',
    errorAddingRound: 'Fel vid skapande av runda',
    dateEnd: 'Slutdatum (valfritt)',
    startTime: 'Starttid (valfritt)',
    subscribeCalendar: 'Prenumerera på kalender',
    calendar: 'Kalender',
    events: 'Övrigt (Kalender)',
    ompc: 'OMPC',
    addEvent: 'Lägg till händelse',
    eventTitle: 'Titel',
    noEventsYet: 'Inga händelser än.',
    deleteEventConfirm: 'Radera händelsen "{title}"?',
    errorAddingEvent: 'Fel vid skapande av händelse',
    deleteRoundConfirm: 'Radera runda {round} ({date})? Alla scorer för denna runda tas bort.',
    selectRound: 'Välj runda',
    selectRoundPlaceholder: 'Välj runda',
    enterNetStrokesHelp: 'Ange nettoslag för varje spelare. Lämna tomt för frånvarande spelare, de får automatiskt högsta score + 1.',
    absentPlaceholder: 'frånvarande',
    saveScores: 'Spara scorer',
    saved: 'Sparat!',
    selectRoundAbove: 'Välj en runda ovan för att registrera scorer.',
    noRoundsCreateFirst: 'Inga rundor finns än. Skapa rundor först.',
    ompcAdminTitle: 'OMPC Admin',
    ompcPlayerTitle: 'OMPC',
    ompcAdminIntro: 'Skapa säsongens cup, lägg till spelare och placera dem i trädet.',
    ompcPlayerIntro: 'Följ säsongens cupträd och rapportera vinnaren i din egen match.',
    loadSeason: 'Ladda säsong',
    noOmpcCupForSeason: 'Ingen OMPC-cup finns för den här säsongen än.',
    ompcParticipants: 'Deltagare',
    ompcParticipantsHelp: 'Lägg till medlemmar i cupen. Placera dem sedan i trädet nedan.',
    ompcInCup: 'I cupen',
    selectMember: 'Välj medlem',
    selectPlayer: 'Välj spelare',
    addParticipantToCup: 'Lägg till deltagare',
    createOmpcCup: 'Skapa OMPC-cup',
    generateBracketTree: 'Generera cupträd',
    resetOmpcSeason: 'Nollställ OMPC-säsong',
    resetting: 'Nollställer...',
    ompcBracket: 'Träd',
    ompcBracketAdminHelp: 'Varje kolumn är ett OMPC-steg. Använd väljare för att placera spelare i varje match.',
    ompcBracketPlayerHelp: 'Endast spelare i en match kan rapportera eller återställa resultatet.',
    deadline: 'Deadline',
    notSet: 'Ej satt',
    winnerLabel: 'Vinnare: {name}',
    saving: 'Sparar...',
    iWon: 'Jag vann',
    playerWon: '{name} vann',
    resetResult: 'Återställ resultat',
    p1Won: 'P1 vann',
    p2Won: 'P2 vann',
    reset: 'Återställ',
    matchSingular: 'match',
    matchPlural: 'matcher',
    final: 'Final',
    semifinals: 'Semifinaler',
    quarterfinals: 'Kvartsfinaler',
    roundOf: 'Omgång med {size}',
    winnerOfMatch: 'Vinnare av match {match}',
    unassigned: 'Ej tilldelad',
    assignedToMatch: 'Tilldelad till match {match} {slot}',
    couldNotLoadOmpcCup: 'Kunde inte läsa in OMPC-cupen.',
    couldNotCreateOmpcCup: 'Kunde inte skapa OMPC-cupen.',
    couldNotResetOmpcSeason: 'Kunde inte nollställa OMPC-säsongen.',
    couldNotAddParticipant: 'Kunde inte lägga till deltagare.',
    couldNotGenerateBracket: 'Kunde inte generera trädet.',
    couldNotUpdateBracketSlot: 'Kunde inte uppdatera plats i trädet.',
    couldNotUpdateMatchResult: 'Kunde inte uppdatera matchresultatet.',
    couldNotResetMatchResult: 'Kunde inte återställa matchresultatet.',
    couldNotUpdateRoundDeadline: 'Kunde inte uppdatera deadline för omgången.',
    resetOmpcSeasonConfirm: 'Nollställ OMPC-säsong {season}? Detta tar bort cupen, deltagarna, matcherna och deadlines.',
  },
  en: {
    language: 'Language',
    swedish: 'Swedish',
    english: 'English',
    standings: 'Standings',
    rounds: 'Rounds',
    members: 'Members',
    enterScores: 'Enter Scores',
    changePassword: 'Change password',
    logout: 'Log out',
    seasonStandings: 'Season Standings',
    season: 'Season',
    loading: 'Loading...',
    noRoundsForSeason: 'No rounds have been created for {season} yet.',
    player: 'Player',
    best4: 'Best 4',
    best4Legend: 'The best four results are highlighted in green.',
    absentLegend: 'Absent players get the highest score in the round + 1.',
    noSeasonsFound: 'No seasons found. An admin needs to create rounds first.',
    round: 'Round',
    date: 'Date',
    course: 'Course',
    notes: 'Notes',
    viewScores: 'View scores',
    backToRounds: 'Back to rounds',
    netStrokes: 'Net strokes',
    absent: 'Absent',
    noScoresYet: 'No scores have been entered for this round yet.',
    email: 'Email',
    password: 'Password',
    continue: 'Continue',
    firstLoginHelp: 'For first login, enter your email and continue. We will send a verification link so you can set your password.',
    firstLoginEmailSent: 'We sent a verification link to {email}. The link is valid for 24 hours.',
    passwordSetupFromEmail: 'You opened the verification link. Set your new password to continue.',
    forgotPassword: 'I forgot my password',
    enterEmailFirst: 'Enter your email first',
    passwordResetEmailSent: 'If that email exists, a reset link has been sent. The link is valid for 24 hours.',
    couldNotSendPasswordResetEmail: 'Could not send password reset email',
    newPassword: 'New password',
    repeatPassword: 'Repeat password',
    setPassword: 'Set password',
    invalidEmailOrPassword: 'Invalid email or password',
    passwordsDoNotMatch: 'Passwords do not match',
    couldNotCompleteFirstLogin: 'Could not complete first login',
    currentPassword: 'Current password',
    repeatNewPassword: 'Repeat new password',
    passwordChanged: 'Password changed!',
    errorChangingPassword: 'Error changing password',
    save: 'Save',
    name: 'Name',
    addMember: 'Add Member',
    fullName: 'Full name',
    golfId: 'Golf ID',
    handicap: 'Handicap',
    admin: 'Admin',
    verified: 'Verified',
    active: 'Active',
    yes: 'Yes',
    no: 'No',
    cancel: 'Cancel',
    edit: 'Edit',
    resetLogin: 'Reset login',
    delete: 'Delete',
    deleteMemberConfirm: 'Delete member "{name}"? This also removes the player\'s scores.',
    resetLoginConfirm: 'Reset login for "{name}"? The member will need to complete first login again.',
    manageRounds: 'Manage Rounds',
    addRound: 'Add Round',
    roundNumber: 'Round #',
    noRoundsYet: 'No rounds yet.',
    errorAddingRound: 'Error adding round',
    dateEnd: 'End date (optional)',
    startTime: 'Start time (optional)',
    subscribeCalendar: 'Subscribe to calendar',
    calendar: 'Calendar',
    events: 'Events (Calendar)',
    ompc: 'OMPC',
    addEvent: 'Add Event',
    eventTitle: 'Title',
    noEventsYet: 'No events yet.',
    deleteEventConfirm: 'Delete event "{title}"?',
    errorAddingEvent: 'Error adding event',
    deleteRoundConfirm: 'Delete round {round} ({date})? All scores for this round will be deleted.',
    selectRound: 'Round',
    selectRoundPlaceholder: 'Select round',
    enterNetStrokesHelp: 'Enter net strokes for each player. Leave blank for absent players, who will automatically receive the highest score + 1.',
    absentPlaceholder: 'absent',
    saveScores: 'Save Scores',
    saved: 'Saved!',
    selectRoundAbove: 'Select a round above to enter scores.',
    noRoundsCreateFirst: 'No rounds exist yet. Create rounds first.',
    ompcAdminTitle: 'OMPC Admin',
    ompcPlayerTitle: 'OMPC',
    ompcAdminIntro: 'Create the season cup, add players, then place them into bracket slots with dropdown selectors.',
    ompcPlayerIntro: 'Follow the season cup bracket and report the winner of your own match.',
    loadSeason: 'Load season',
    noOmpcCupForSeason: 'No OMPC cup exists for this season yet.',
    ompcParticipants: 'Participants',
    ompcParticipantsHelp: 'Add members to the cup. Then assign them to bracket slots from the dropdowns below.',
    ompcInCup: 'In Cup',
    selectMember: 'Select member',
    selectPlayer: 'Select player',
    addParticipantToCup: 'Add participant',
    createOmpcCup: 'Create OMPC cup',
    generateBracketTree: 'Generate bracket tree',
    resetOmpcSeason: 'Reset OMPC season',
    resetting: 'Resetting...',
    ompcBracket: 'Bracket',
    ompcBracketAdminHelp: 'Each column is one OMPC stage. Use the slot selectors to place players in each match.',
    ompcBracketPlayerHelp: 'Only players in a match can report or reset its result.',
    deadline: 'Deadline',
    notSet: 'Not set',
    winnerLabel: 'Winner: {name}',
    saving: 'Saving...',
    iWon: 'I won',
    playerWon: '{name} won',
    resetResult: 'Reset result',
    p1Won: 'P1 won',
    p2Won: 'P2 won',
    reset: 'Reset',
    matchSingular: 'match',
    matchPlural: 'matches',
    final: 'Final',
    semifinals: 'Semifinals',
    quarterfinals: 'Quarterfinals',
    roundOf: 'Round of {size}',
    winnerOfMatch: 'Winner of Match {match}',
    unassigned: 'Unassigned',
    assignedToMatch: 'Assigned to Match {match} {slot}',
    couldNotLoadOmpcCup: 'Could not load OMPC cup.',
    couldNotCreateOmpcCup: 'Could not create OMPC cup.',
    couldNotResetOmpcSeason: 'Could not reset OMPC season.',
    couldNotAddParticipant: 'Could not add participant.',
    couldNotGenerateBracket: 'Could not generate bracket.',
    couldNotUpdateBracketSlot: 'Could not update bracket slot.',
    couldNotUpdateMatchResult: 'Could not update match result.',
    couldNotResetMatchResult: 'Could not reset match result.',
    couldNotUpdateRoundDeadline: 'Could not update round deadline.',
    resetOmpcSeasonConfirm: 'Reset OMPC season {season}? This removes the cup, participants, matches, and deadlines.',
  },
}

function interpolate(text, params = {}) {
  return text.replace(/\{(\w+)\}/g, (_, key) => params[key] ?? '')
}

export const useLocaleStore = defineStore('locale', () => {
  const locale = ref(localStorage.getItem('locale') || 'sv')

  watch(locale, value => {
    localStorage.setItem('locale', value === 'en' ? 'en' : 'sv')
  })

  function setLocale(value) {
    locale.value = value === 'en' ? 'en' : 'sv'
  }

  function t(key, params = {}) {
    const language = translations[locale.value] || translations.sv
    const fallback = translations.en[key] || key
    return interpolate(language[key] || fallback, params)
  }

  return { locale, setLocale, t }
})
